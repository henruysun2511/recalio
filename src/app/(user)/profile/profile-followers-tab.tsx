...
import { useUserFollowers, useFollowUser, useUnfollowUser } from "@/queries/useFollowQuery"

const PAGE_SIZE = 10

export function ProfileFollowersTab({ userId }: { userId: string }) {
    const [search, setSearch] = useState("")
    const [page, setPage] = useState(1)

    const { data: res, isLoading } = useUserFollowers(userId, { page, limit: PAGE_SIZE, sortOrder: SortOrder.DESC })
    ...
    return (
        <div className="space-y-4">
            ...
            {isLoading ? (
                ...
                <EmptyState title="Không có người theo dõi" description={search ? "Thử tìm kiếm với từ khóa khác." : "Chưa có ai theo dõi bạn."} />
            ) : (
